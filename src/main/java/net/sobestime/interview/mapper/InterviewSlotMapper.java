...
public class InterviewSlotMapper {
    ...
    public UserInterviewSlotDto toUserSlotDto(InterviewSlot slot) {
        InterviewRequest request = slot.getRequest();
        return UserInterviewSlotDto.builder()
            .interviewId(request.getId())
            .slotId(slot.getId())
            .interviewTitle(request.getTitle())
            .status(statusMapper.toDto(slot.getStatus()))
            .startTime(dateTimeService.toOffsetDateTime(slot.getStartTime()))
            .creator(interviewUserMapper.toDto(slot.getBooker()))
            .build();
    }
    ...
    public List<UserInterviewSlotDto> toUserSlotDto(List<InterviewSlot> items) {
        return items.stream().map(this::toUserSlotDto).collect(Collectors.toList());
    }
}
